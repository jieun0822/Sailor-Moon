...
HRESULT minimap::init()
{
	...
	IMAGE->addImage("배경화면", "images/BG1.bmp", WINSIZEX * 4, WINSIZEY,true, RGB(255,0,255));
	IMAGE->addImage("카메라", "images/카메라.bmp", WINSIZEX / 10, WINSIZEY / 10, true, RGB(255, 0, 255));
	IMAGE->addImage("미니맵", "images/미니맵.bmp", WINSIZEX *2/10 , WINSIZEY/10, 50,50,true, RGB(255, 0, 255));
	...
	//몬스터 상태
	IMAGE->addFrameImage("Bret_Butler", "images/bret_butler.bmp", 18 * 2, 18 * 2, 1, 1, true, RGB(255, 0, 255));
	...
	_monster = IMAGE->findImage("Bret_Walk_Left");
...
void minimap::update()
{
	...
	if (!isPlayerDie)
	{
		if (InputManager->isStayKeyDown(VK_LEFT) && player1_x - _player->getFrameWidth() / 2 > 0 &&currentState != State::JUMP)
		{
		...
		else if (InputManager->isStayKeyDown(VK_RIGHT) && player1_x + _player->getFrameWidth() / 2 < WINSIZEX && currentState != State::JUMP)
		{
		...
		if (InputManager->isOnceKeyUp(VK_LEFT) && currentState != State::JUMP) currentState = State::IDLE_LEFT;
		if (InputManager->isOnceKeyUp(VK_RIGHT) && currentState != State::JUMP) currentState = State::IDLE_RIGHT;
		...
		if (InputManager->isOnceKeyDown(VK_UP))
		{
			...
			if (prevState == State::WALK_RIGHT ||
				prevState == State::IDLE_RIGHT) m_index = 0;
			else if (prevState == State::WALK_LEFT ||
				prevState == State::IDLE_LEFT) m_index = 11;
		}

		else if (InputManager->isOnceKeyDown('Z') && currentState != State::JUMP)//캐릭터1_공격1.
		{
			currentState = State::ATTACK1;
			_player = IMAGE->findImage("캐릭터1_공격1");

			if (prevState == State::WALK_RIGHT) m_index = 0;
			else if (prevState == State::WALK_LEFT) m_index = 4;
		}

		else if (InputManager->isOnceKeyDown('X') && currentState != State::JUMP)//캐릭터1_공격2.
		{
		...
		else if (InputManager->isOnceKeyDown('C') && currentState != State::JUMP 
		...
		else if (InputManager->isOnceKeyDown('A') && currentState != State::JUMP
			&& (playerMP_Front.left + 50 <= playerMP_Back.right))//캐릭터1_공격5.
		{
			currentState = State::ATTACK5;
			_player = IMAGE->findImage("캐릭터1_공격5");

			if (prevState == State::WALK_RIGHT) m_index = 0;
			else if (prevState == State::WALK_LEFT) m_index = 6;
	...
	//몬스터 조작.
	if (isPlayerDie)monsterState = monsterState::WALK;
	...
	if (!isMonsterDie && !isPlayerDie)
	{
		...
		if (monster_term > 5)
		{
			...
			{
				_monster = IMAGE->findImage("Bret_Walk_Left");
				if (monster_x - _monster->getFrameWidth() / 2 > -250) monster_x -= 10;
				else monster_isLeft = false;
			}
			else
			{
				_monster = IMAGE->findImage("Bret_Walk_Right");
				if (monster_x + _monster->getFrameWidth() / 2 < WINSIZEX - 150) monster_x += 10;
		...
		if (count > 10)
		{
			RECT temp;
			//플레이어 공격에 맞았을때.
			if ((currentState == State::ATTACK1 || currentState == State::ATTACK2) && IntersectRect(&temp, &_playerRC, &_monsterRC))
			{
			...
			else if (monster_collision) monsterState = monsterState::DAMAGE;
			else monsterState = monsterState::WALK;
			count = 0;
		}
	}
	...
	case State::JUMP:
		...
		if (prevState == State::WALK_RIGHT)
		{
			...
			if (m_count % 5 == 0)
			{
				...
				if (m_index > 11) currentState = State::IDLE_RIGHT;
	...
	case State::DAMAGE1:
		...
		if (prevState == State::WALK_RIGHT)
		{
			...
			if (m_count % 5 == 0)
			{
				...
				_player->setFrameX(2);
				if(monsterState != monsterState::ATTACK)currentState = State::IDLE_RIGHT;
			}
		}
		else
		{
			...
			if (m_count % 5 == 0)
			{
				...
				_player->setFrameX(3);
				if (monsterState != monsterState::ATTACK)currentState = State::IDLE_LEFT;
			}
		}
		break;
	}
...
void minimap::render()
{
	...
	if (!isMonsterDie)
	{
		...
		{
		case monsterState::WALK:
			if (monster_isLeft) IMAGE->frameRender("Bret_Walk_Left", getMemDC(), monster_x, monster_y - 100, _monster->getFrameX(), _monster->getFrameY());
			else IMAGE->frameRender("Bret_Walk_Right", getMemDC(), monster_x, monster_y - 100, _monster->getFrameX(), _monster->getFrameY());
			break;
		case monsterState::DAMAGE:
			if (monster_isLeft) IMAGE->frameRender("Bret_Damage_Left", getMemDC(), monster_x, monster_y - 100, _monster->getFrameX(), _monster->getFrameY());
			else IMAGE->frameRender("Bret_Damage_Right", getMemDC(), monster_x, monster_y - 100, _monster->getFrameX(), _monster->getFrameY());
		...
		case monsterState::ATTACK:
			...
			if (player1_x - 100 < monster_x) IMAGE->frameRender("BretAttack_Left", getMemDC(), monster_x, monster_y - 110, _monster->getFrameX(), _monster->getFrameY());
			else IMAGE->frameRender("BretAttack_Right", getMemDC(), monster_x, monster_y - 110, _monster->getFrameX(), _monster->getFrameY());
			break;
		}
	}
	...
	{
		IMAGE->frameRender("Bret_Butler", getMemDC(), m_gaugeBack.left-45, m_gaugeBack.top-8, _monsterState->getFrameX(), _monsterState->getFrameY());
	...
	//캐릭터 상태창
	if(currentState == State::DAMAGE1 || isPlayerDie) IMAGE->frameRender("캐릭터상태창_데미지", getMemDC(), 10, 10, _playerState->getFrameX(), _playerState->getFrameY());
...
void minimap::scoreRender()
{
	...
	{
		SetTextColor(getMemDC(), RGB(102,102, 102));
		TCHAR str[100];
...
void minimap::timeRender()
{
	...
	int time10 = (60 - time) / 10;
	int time1 = (60 - time) % 10;
...
void minimap::GetBuyItem(int id)
{
	for (_viInventory = _vInventory.begin(); _viInventory != _vInventory.end(); ++_viInventory)
	{
